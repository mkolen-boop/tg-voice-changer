...
def apply_phone_effect(mp3_bytes: bytes) -> bytes:
    ...
    subprocess.run(
        [
            ...
            "highpass=f=150,lowpass=f=9000,volume=5.0",
            out_path,
        ],
        check=True, capture_output=True,
    )
...
async def handle_start(message: Message):
    await message.answer(
        "Привет! Отправь голосовое или напиши текст — я озвучу его нужным голосом.\n\n"
        "📌 Паузы\n"
        "  ,  — короткая пауза\n"
        "  „  — длинная пауза\n"
        "  —  — значительная пауза\n\n"
        "📌 Акценты\n"
        "  СЛОВО — произнесётся громче и с акцентом\n"
        "  СЛОВО! — ещё сильнее\n\n"
        "📌 Интонация\n"
        "  ?  — вопросительная интонация\n"
        "  ??? — более выраженная вопросительная интонация\n\n"
        "📌 Ударение\n"
        "  Если модель ставит неверное ударение — напиши ударную букву заглавной.\n"
        "  Например: «молодЕц» вместо «молодец»\n\n"
        "📌 Точки\n"
        "  .  — чтобы закончить мысль\n"
        "  ...  — задумчивая, незавершённая интонация\n\n"
        "🔄 Кнопка «Перегенерировать»\n"
        "  Нейросеть каждый раз генерирует по-разному — иногда второй или третий результат выходит лучше."
    )
